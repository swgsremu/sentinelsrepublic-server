...
int SRStructureTerminalMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* creature, byte selectedID) const {
    debug() << "SRStructureTerminalMenuComponent: handleObjectMenuSelect called with selectedID: " << (int)selectedID;
    
    if (selectedID != SRRadialIds::PACK_UP_STRUCTURE) {
        debug() << "SRStructureTerminalMenuComponent: selectedID " << (int)selectedID << " != PACK_UP_STRUCTURE " << (int)SRRadialIds::PACK_UP_STRUCTURE << ", calling base handler";
        return StructureTerminalMenuComponent::handleObjectMenuSelect(sceneObject, creature, selectedID);
    }

    info() << "SRStructureTerminalMenuComponent: Pack up structure selected by player: " << creature->getFirstName();
